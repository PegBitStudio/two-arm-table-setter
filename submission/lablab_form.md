...
**Intel optimisation.** Measured on an Intel Core Ultra 7 155H across all three of its chips. OpenVINO runs the ACT policy 2.9× faster than PyTorch on the CPU (0.72 ms vs 2.07 ms per call) with no loss of task success, at FP32, FP16, INT8 and INT8-weights. The same policy runs on the **AI Boost NPU** at 0.90 ms per call at FP32 — and at FP16 there it places 10/10 mugs — freeing the CPU and iGPU. Command understanding runs on the **Arc iGPU** in 1.3 s per command — 2.8× faster than the CPU — after cutting it from 30 s with smaller pictures and shorter answers. We also report an honest negative result: OpenVINO 2026.3's NPU compiler cannot build Qwen3-VL-4B's vision tower, so the language model falls back to the iGPU automatically.
...
- Pitch video (narrated, 2:56): **https://youtu.be/3iH_Mt60lbo** (unlisted) — source `submission/pitch_video.mp4`.
